...
class TrainingObjects:
    def __init__(self, npts, neurons,max_freq, activation="tanh"):
        
        #Create model
        self.u_model = build_model(neurons,activation)
        
        #Define optimiser
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=10**-4)
        ...
        self.npts = npts
        self.h=np.pi/npts
        self.x_mesh = tf.constant([np.pi*i/npts for i in range(npts)]) 
        
        ##We will estimate the error during training. We use a fixed mesh
        self.x_error = tf.constant([np.pi*(i+0.5)/200 for i in range(200)])
        
        self.klist = tf.constant([k*1. for k in range(1,max_freq+1)])
    ...
    def rhs(self,x):
        return -9*tf.math.sin(3*x)
    
    ##The exact solution for comparison
    
    def u_exact(self,x):
        return tf.math.sin(3*x)
    ...
    def loss_fn(self):
        ...
        X,K = tf.meshgrid(x,self.klist)
        
        d_basis_functions = tf.math.cos(K*X)
        
        basis_functions = tf.einsum("kx,k->kx",tf.math.sin(K*X),self.klist**-1)
        
        residual_transform_derivs = tf.einsum("kx,x->k",d_basis_functions,du)/self.npts
        residual_transform_rhs = tf.einsum("kx,x->k",basis_functions,self.rhs(x))/self.npts
        
        
        ##Combine into the loss
        loss = tf.reduce_sum( (residual_transform_derivs+residual_transform_rhs)**2)
...
T = TrainingObjects(200,50,neurons)
T.train(iterations)
...
xplot = tf.constant([np.pi*i/100 for i in range(101)])
